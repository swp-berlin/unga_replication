...
ptm <-
  proc.time()	## Record processor time at beginning of session
...
# FileSuffix 		= paste(DataCode, "_replication_2024_15", sep = "")		## Include data and any subsetting information
...
## Load data
AllData = read.table(file 	= file.path(Path, "Output",str_c("AllData_", FileSuffix, ".txt") ))
CSession	= AllData[, 1]
Country	= AllData[, 2]
Session	= AllData[, 3]
VoteID	= AllData[, 5]
...
LagThetaPrior = ThetaMean[match(CountryList + (SessionList - 1) /
                                          100,
                                        CountryList + SessionList / 100)]
LagThetaPrior[is.na(LagThetaPrior) == 1 &
                SessionList == 20]	= LagThetaPrior [ThetaMeanObs[is.na(LagThetaPrior) ==
                                                                   1 & SessionList == 20] - 1]
LagThetaPrior[is.na(LagThetaPrior) == 1]				= 0

for (cc in 1:length(UniqueCountry)) 	{
  LagThetaPrior[CountryList == UniqueCountry[cc] &
                  SessionList == MinCountryYear[cc]] =  ThetaMean[CountryList == UniqueCountry[cc] &
                                                                    SessionList == min(SessionList[CountryList == UniqueCountry[cc] &
                                                                                                     SessionList > MinCountryYear[cc]])]
} # END for (cc in 1:leng 		# Set prior on first year to be ideal point from second year in previous MCMC iteration

for (nn in 1:NN){
  if (GapYear[nn] == 1)
    LagThetaPrior[nn] = ThetaMean[CountryList == CountryList[nn] &
                                    SessionList == max(SessionList[CountryList == CountryList[nn] &
                                                                     SessionList < SessionList[nn]])]}

# For countries with gap in sessions, use most recent session as LagThetaPrior
for (ii in 1:NN) {
  indexValue 		= IObsMat[ii, 1:IndN[ii]]
...
## STEP 2: Generate candidate gammas for updating Gamma1, Gamma2 (See Johnson and Albert, p. 136) [NOTE: these Gamma parameters are actually Gamma*Beta]
	...
	U 	= runif(ObsN)
	Z 	= BetaVector * ThetaVector + qnorm(pnorm(gLo - BetaVector * ThetaVector) +  U * (
	  pnorm(gHi - BetaVector * ThetaVector) - pnorm(gLo - BetaVector * ThetaVector)
	))
	#CHECK if(max(Z)> 100) stop("Max Z >100")
	Z[Z > 9]	= 9
	Z[Z < -9]	= -9
	## CHECK Z[VoteStartEnd[tt,1]:VoteStartEnd[tt,2]]
	...
	for (tt in 1:TT)	{
	  VarBeta[tt] 	= 1 / (sum(ThetaVector[VoteStartEnd[tt, 1]:VoteStartEnd[tt, 2]] ^
	                            2)	+ 1 / S2BetaPrior)
	  BetaMean[tt]	= VarBeta[tt] * sum(ThetaVector[VoteStartEnd[tt, 1]:VoteStartEnd[tt, 2]] * Z[VoteStartEnd[tt, 1]:VoteStartEnd[tt, 2]]) + BetaPrior /
	    S2BetaPrior
	} ## END: for (tt in 1:TT)	{
	Beta	 	= BetaMean + sqrt(VarBeta) * zrnorm(TT)	## Uses zrnorm from RcppZiggurat package (for speed)
	
	if (max(abs(Beta)) > 10)
	  print(c(kk, "Abs(Max Beta) >10"))
	BetaVector 	= rep(Beta, VoteN)
...
VoteMatrix = cbind(1:length(t(BetaEst)), VoteTally, VoteCode, BetaEst, Gamma1Est, Gamma2Est)
names(VoteMatrix)[1:11] = c(
  "tt",
  "rcidL",
  "Yes",
  "Abstain",
...
print("CPU time in hours is ")	#proc.time() - ptm
(proc.time() - ptm)/60/60		## Current processor time minus time at the beginning of the session
