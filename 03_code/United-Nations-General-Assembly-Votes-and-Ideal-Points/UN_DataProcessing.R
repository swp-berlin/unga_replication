...
completeVotes$new <-
  if_else(
    completeVotes$year > 1989 &
      (completeVotes$para == 1 |
         completeVotes$amend == 1),
    1,
    0,
    missing = 0
  )

### filter to our category of peace and security
completeVotes <- filter(completeVotes, new == 0)
UNall	= completeVotes[, c(
  "rcid",
  "ccode",
  "member",
  "vote",
  ...
  "he",
  "pc"
)]   ## was "df" in other versions
UNall[1:5,]
UNall[is.na(UNall$rcid) == 1, ][1:5, ]


## Set single South Africa 1993 (only 1 observation after being out of UN since 1974)
SouthAfricaCut = as.numeric(UNall$ccode == 560 &
                              UNall$session == 48 & UNall$vote < 4)	## table(SouthAfricaCut)
...
if (DataCode == "All") 		{
  UN	= UNall[which(UNall$vote < 4 &
                     UNall$session != 19 & UNall$ccode != 511),]
}	## which prevents NAs from entering

if (DataCode == "health") 		{
  UN	= UNall[which(UNall$vote < 4 &
                     UNall$session != 19 &
                     UNall$ccode != 511),] |> filter(he == 1)
}	## which prevents NAs from entering
...
for (rr in 0:(NMatch - 1)) {
  UN$rcidL[UN$rcidL == Matched$rcid2[NMatch - rr]] = min(Matched$rcid1[Matched$rcid2 == Matched$rcid2[NMatch -
                                                                                                        rr]])
}
...
for (tt in 1:TTlink) {
  if (tt %% 300 == 0)
    print(tt)
  VoteTally[tt, 2] = length(UN$vote[UN$rcidL == VoteList[tt] &
                                      UN$vote == 1])
  VoteTally[tt, 3] = length(UN$vote[UN$rcidL == VoteList[tt] &
                                      UN$vote == 2])
  VoteTally[tt, 4] = length(UN$vote[UN$rcidL == VoteList[tt] &
                                      UN$vote == 3])
}	# END for(ii in 1:TTlink){
VoteTally[, 5]	= apply(VoteTally[, 2:4], 1, sum)
VotePct 		= VoteTally[, 2:4] / VoteTally[, 5]
UnamVoteList	= VoteTally[VotePct[, 1] == 1.0 |
                           VotePct[, 2] == 1.0 | VotePct[, 3] == 1.0, 1]
CutCode 		= 0 * UN$vote
for (cc in 1:length(UnamVoteList))
  CutCode[UN$rcidL == UnamVoteList[cc]] = 1

UN2			= UN[CutCode == 0, ]	## Cut unanimous votes
UN2$CSession 	= round(UN2$ccode + UN2$session / 100, 2)
AllData 		= data.frame(UN2$CSession,
                       UN2$ccode,
                       UN2$session,
                       UN2$rcid,
                       UN2$rcidL,
                       UN2$vote)	#, UN2$voetenshortcode
AllData 		= AllData[order(UN2$rcidL, UN2$ccode, UN2$session),]			## Sort data by (linked) roll call number (rcidL)

readr::write_delim(AllData,delim = " ",
      file = file.path(Path, "Output", str_c("AllData_", FileSuffix, ".txt")),col_names = FALSE)
...
for (tt in 1:TTlink) {
  if (tt %% 300 == 0)
    print(tt)
  VoteTally[tt, 2] = length(UN2$vote[UN2$rcidL == VoteList[tt] &
                                       UN2$vote == 1])
  VoteTally[tt, 3] = length(UN2$vote[UN2$rcidL == VoteList[tt] &
                                       UN2$vote == 2])
  VoteTally[tt, 4] = length(UN2$vote[UN2$rcidL == VoteList[tt] &
                                       UN2$vote == 3])
}	# END for(ii in 1:TTlink){
VoteTally[, 5]	= apply(VoteTally[, 2:4], 1, sum)
VotePct 		= VoteTally[, 2:4] / VoteTally[, 5]
...
readr::write_delim(VoteTally |> as.data.frame(),delim = " ",
                   file = file.path(Path, "Output",str_c("VoteTally_", FileSuffix, ".txt")),
                   col_names = FALSE)

readr::write_delim(VoteList |> as.data.frame(),delim = " ",
...
for (tt in 1:TTlink)	{
  ## This is possible b/c sorted AllData by (linked) roll call number (rcidL) above
...
readr::write_delim(IObsMat |> as.data.frame(),delim = " ",
...
for (nn in 1:length(IndN)) {
  # The weight of smoothing depends on how much data in current year versus previous year (lower values mean more smoothing (weight on previous year theta is higher))
  if (length(UN2$ccode[UN2$ccode == CountryList[nn] &
                       UN2$session == SessionList[nn] - 1]) > 0)
    SmoothVector[nn] 		= IndN[nn] /
      (IndN[nn] + length(UN2$ccode[UN2$ccode == CountryList[nn] &
                                     UN2$session == SessionList[nn] - 1]))
  if (SessionList[nn] == MinCountryYear[match(CountryList[nn], UniqueCountry)])
    SmoothVector[nn] 		= IndN[nn] /
      (IndN[nn] + length(UN2$ccode[UN2$ccode == CountryList[nn] &
                                     UN2$session == min(SessionList[CountryList == CountryList[nn] &
                                                                      SessionList > SessionList[nn]])]))
}
...
for (nn in 1:length(IndN)) {
  if (GapYear[nn] == 1)
    SmoothVector[nn] = IndN[nn] /
      (IndN[nn] + length(UN2$ccode[UN2$ccode == CountryList[nn] &
                                     UN2$session == max(UN2$session[UN2$ccode == CountryList[nn] &
                                                                      UN2$session < SessionList[nn]])]))
}
...
for (tt in 1:TTlink){
  if (length(AllData[AllData[, 2] == 2   &
                     AllData[, 5] == VoteList[tt], 6]) > 0)
    USVote[tt] 		= AllData[AllData[, 2] == 2   &
                             AllData[, 5] == VoteList[tt], 6]
}
UKVote	= rep(NA, TTlink)

for (tt in 1:TTlink){
  if (length(AllData[AllData[, 2] == 200 &
                     AllData[, 5] == VoteList[tt], 6]) > 0)
    UKVote[tt] 		= AllData[AllData[, 2] == 200 &
                             AllData[, 5] == VoteList[tt], 6]}
...
for (tt in 1:TTlink){
  if (length(AllData[AllData[, 2] == 365 &
                     AllData[, 5] == VoteList[tt], 6]) > 0)
    RussiaVote[tt] 	= AllData[AllData[, 2] == 365 &
                                AllData[, 5] == VoteList[tt], 6]}
...
Beta 					= 0 - 1 * (apply(USUK, 1, mean, na.rm = T) == 1)  + (apply(USUK, 1, mean, na.rm =
                                                                       T) > 2.0)
Beta[is.na(Beta) == 1]		= 0
...
for (vvv in 1:TTlink) {
  if (VoteTally[vvv , 2] > 0 &
      VoteTally[vvv , 3] > 0 & VoteTally[vvv , 4] >  0)
    VoteCode[vvv] = 1
  if (VoteTally[vvv , 2] > 0 &
      VoteTally[vvv , 3] > 0 & VoteTally[vvv , 4] == 0)
    VoteCode[vvv] = 2
...
readr::write_delim(VoteCode |> as.data.frame(),delim = " ",
...
gStart 	= cbind(rep(-Inf, TTlink),
                -1 + 2 * VotePct[, 1],
                -1 + 2 * (VotePct[, 1] + VotePct[, 2]),
                rep(Inf, TTlink))
# write(
#   t(gStart),
...
# )	
readr::write_delim(gStart |> as.data.frame(),delim = " ",
                   file = file.path(Path, "Output",str_c("gStart_", FileSuffix, ".txt")),
                   col_names = FALSE)
...
if (NewGammaStart 	== 1) {
  ...
  ThetaSummaryAll		= read.table(file = file.path(Path, "Output",str_c("ThetaSummaryRW_July2015.txt")))		## Manually identify most recent theta results
  ...
  for (tt in 1:TTlink) {
    ## Ensure gamma1 < gamma2 by flipping as needed (around 200 votes or so, typically low beta)
    if (xCANDGamma1[tt] > xCANDGamma2[tt]) {
      tempHold 	= xCANDGamma2[tt]
      xCANDGamma2[tt] = xCANDGamma1[tt]
      xCANDGamma1[tt] = tempHold
    }
  }	# END for(tt in 1:TT)
...
if (CreateAgreeMatrix == 1) {
  ...
  for (jjj in 1:NN) {
    ...
    RUSStempV 	= VoteID[Country == 365  	&
                          Session == SessionList[jjj]]
    RUSStempYY 	= yObs  [Country == 365  	&
                           Session == SessionList[jjj]]
    # Votes in Session on which Brazil has voted
    BraziltempV 	= VoteID[Country == 140  	&
                            Session == SessionList[jjj]]
    BraziltempYY 	= yObs  [Country == 140  	&
                             Session == SessionList[jjj]]
    # Votes in Session on which China has voted
    ChinatempV 		= VoteID[Country == 710  	&
                            Session == SessionList[jjj]]
    ChinatempYY 	= yObs  [Country == 710  	&
                            Session == SessionList[jjj]]
    # Votes in Session on which India has voted
    IndiatempV 		= VoteID[Country == 750  	&
                            Session == SessionList[jjj]]
    IndiatempYY 	= yObs  [Country == 750  	&
                            Session == SessionList[jjj]]
    # Votes in Session on which Israel has voted
    IsraeltempV 	= VoteID[Country == 666  	&
                            Session == SessionList[jjj]]
    IsraeltempYY 	= yObs  [Country == 666  	&
                             Session == SessionList[jjj]]
    # Votes in Session on which Canada has voted
    CanadatempV 	= VoteID[Country == 20  	&
                            Session == SessionList[jjj]]
    CanadatempYY 	= yObs  [Country == 20  	&
                             Session == SessionList[jjj]]
    ...
    yobCountMatrix[jjj, 1] 	= length(yObs[yObs == 1 &
                                            CSession == CSList[jjj]])
    yobCountMatrix[jjj, 2] 	= length(yObs[yObs == 2 &
                                            CSession == CSList[jjj]])
    yobCountMatrix[jjj, 3] 	= length(yObs[yObs == 3 &
                                            CSession == CSList[jjj]])
  ...
  for (tt in 1:TTlink)
    if (length(AllData[AllData[, 2] == 666 &
                       AllData[, 5] == VoteList[tt], 6]) > 0)
      IsraelVote[tt] 	= AllData[AllData[, 2] == 666  &
                                  AllData[, 5] == VoteList[tt], 6]
  
  BrazilVote	= rep(NA, TTlink)
  
  for (tt in 1:TTlink)
    if (length(AllData[AllData[, 2] == 140 &
                       AllData[, 5] == VoteList[tt], 6]) > 0)
      BrazilVote[tt] 	= AllData[AllData[, 2] == 140  &
                                  AllData[, 5] == VoteList[tt], 6]
  
  ChinaVote	= rep(NA, TTlink)
  
  for (tt in 1:TTlink)
    if (length(AllData[AllData[, 2] == 710 &
                       AllData[, 5] == VoteList[tt], 6]) > 0)
      ChinaVote[tt] 	= AllData[AllData[, 2] == 710  &
                                 AllData[, 5] == VoteList[tt], 6]
  ...
  for (tt in 1:TTlink)
    if (length(AllData[AllData[, 2] == 750 &
                       AllData[, 5] == VoteList[tt], 6]) > 0)
      IndiaVote[tt] 	= AllData[AllData[, 2] == 750  &
                                 AllData[, 5] == VoteList[tt], 6]
  
  USUKRussiaBrazilChinaIndiaIsrael		= cbind(USVote,
                                            UKVote,
                                            RussiaVote,
                                            BrazilVote,
                                            ChinaVote,
                                            IsraelVote,
                                            IndiaVote)				## DIAGNOSTIC: VoteSum	= cbind(VoteTally, USVote, UKVote, IsraelVote)
  write(
